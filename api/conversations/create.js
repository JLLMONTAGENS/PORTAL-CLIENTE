...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const open = await sql`SELECT * FROM conversations WHERE phone = ${phone} AND status <> 'closed' LIMIT 1`;
    if (open[0]) return res.status(200).json({ existing: true, conversation: publicConversation(open[0]) });
    const rows = await sql`INSERT INTO conversations (customer_name, phone) VALUES (${name}, ${phone}) RETURNING *`;
    const conversation = rows[0];
    const welcome = 'Aguarde só mais um pouco, que um especialista irá lhe atender.';
    await sql`INSERT INTO messages (conversation_id, sender_type, body) VALUES (${conversation.id}, 'system', ${welcome})`;
