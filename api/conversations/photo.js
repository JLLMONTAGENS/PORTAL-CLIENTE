...
module.exports = async function handler(req, res) {
  ...
  const fileName = String(req.body?.fileName || 'foto').replace(/[^a-zA-Z0-9._-]/g, '-').slice(0, 100);
  const encoded = String(req.body?.data || '');
  if (!conversationId || !allowedTypes.has(mimeType) || !encoded) return res.status(400).json({ error: 'Envie uma imagem válida.' });
